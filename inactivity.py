...
async def on_command_help(message):
    ...
    '''
    try:
        await message.channel.send(embed=discord.Embed(color=color, desciption=
    ...
    for member in guild_data['members']:
        ...
        try:
            if member['name'] in players_today:
                ...
                uuid, name = get_uuid(member['name'], time_now)
                if uuid is None:
                    times[-1] = ["Unavailable"]
                    if -1 in times:
                        times[-1].append("Unavailable")
                    else:
                        times[-1] = ["Unavailable"]
                    continue
                ...
                players_today[member['name']]['name'] = name
